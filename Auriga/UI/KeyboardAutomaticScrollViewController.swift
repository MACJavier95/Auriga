//
//  KeyboardAutomaticScrollViewController.swift
//  Auriga
//
...
extension UIViewController {
    ...
    @objc func keyboardWillShow(_ notification: Notification) {
        ...
        UIView.animate(withDuration: animationDuration, delay: 0, options:options, animations: { () -> Void in
            ...
            self.internalScrollView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: insetHeight + 5, right: 0)
            self.internalScrollView.scrollIndicatorInsets  = UIEdgeInsets(top: 0, left: 0, bottom: insetHeight + 5, right: 0)
        }) { (complete) -> Void in
        }
    }
